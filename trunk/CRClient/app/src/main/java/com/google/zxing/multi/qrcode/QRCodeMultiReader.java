...
public final class QRCodeMultiReader extends QRCodeReader implements MultipleBarcodeReader {
  ...
  private static final class SAComparator implements Comparator<Result>, Serializable {
    ...
    public int compare(Result a, Result b) {
      String strTmp;
      strTmp = (a.getResultMetadata().get(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE)).toString();
      int aNumber = (int) Integer.parseInt(strTmp);
      strTmp = (b.getResultMetadata().get(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE)).toString();
      int bNumber = (int) Integer.parseInt(strTmp);
      if (aNumber < bNumber) {
        return -1;
      }
      if (aNumber > bNumber) {
